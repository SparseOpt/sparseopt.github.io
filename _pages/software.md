...
---
  ...
  <b> Two general forms of sparse optimization: </b> 
   ...
   \begin{eqnarray*}
   \begin{array}{lll}
   \textit{Sparsity constrained optimization:}~&~\min_{x}&~f(x), ~ {\rm s.t.}, ~ \Vert x \Vert_0\leq s \\
   \textit{L0 regularized optimization:} &~\min_{x}&~f(x) +\lambda \Vert x \Vert_0,
   \end{array}
   \end{eqnarray*}
...
---
  ...
  <a href="https://biopt.github.io/solvers/">three solvers</a> and several useful tools. <br>  <br> 
