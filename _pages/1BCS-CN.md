...
<div style="text-align:justify;">
其中， 正整数 $s\ll n$ 和 $k\ll m$， 参数 $(\epsilon，\eta，\varepsilon，\lambda)>0$ 且 $\mathbf{z}_+$=$(\max\{0,z_1\}$ $\cdots$ $\max\{0,z_m\})^\top$。度量 $\parallel \mathbf{z}_+\parallel_0$ 与阶梯（又称 0/1 损失）函数相关，该函数定义为 $\mathrm{step}(t)=1$ 当 $t>0$，否则 $\mathrm{step}(t)=0$。因此，$\|\mathbf{z}_+\|_0$=$\mathrm{step}(z_1)$+$\cdots$+$\mathrm{step}(z_m)$ 表示 $\mathbf{z}$ 中正元素的个数。
</div> 
