...
---
<div style="text-align:justify;">
The citation for CSpack is shown below. Here, inputs $(\texttt{A},\texttt{b},n,\texttt{solver})$ are required. If $\texttt{A}$ is a function handle, then $\texttt{At}$ is required. If $\texttt{A}$ is a matrix,  $\texttt{At}$ can be $\texttt{A}'$ or $\texttt{[]}$. If \texttt{solver} is one of \texttt{{'NHTP','GPNP','IIHT'}}, then $s$ is required. If \texttt{solver} is one of \texttt{{'PSNP','NL0R','MILR1'}}, then $s$ can be $\texttt{[]}$.
</div>
