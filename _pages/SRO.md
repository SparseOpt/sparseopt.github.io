...
where $f:\mathbb{R}^{\mathrm{n}}\rightarrow \mathbb{R}$ is a continuously or twice continuously differentiable function, $\lambda>0$ is the penalty parameter, and $\parallel\mathbf{x}\parallel_{\mathrm{q}}^{\mathrm{q}}\mathrm{=|x_1|^q+\cdots+|x_n|^q}$ denotes the Lq norm with $\mathrm{0≤q<1}$, in particular, $\parallel\mathbf{x}\parallel_0:=\parallel\mathbf{x}\parallel_0^0$ denotes the L0 norm, which counts the number of nonzero entries in $\mathbf{x}$.
</div>
