...
其中，函数 $f:\mathbb{R}^{n}\rightarrow \mathbb{R}$ 连续可微，最好是二次连续可微，$s$ 是一个远小于 $n$ 的正整数, $\|\mathbf{x}\|_0$ 为 $\mathbf{x}$ 的零范数，计算 $\mathbf{x}$ 中非零元个数。
</div>
